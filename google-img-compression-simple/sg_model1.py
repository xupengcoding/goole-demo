...
tf.app.flags.DEFINE_string('data_dir', 'image_net/', 
                            """path to train data.""")
...
def _variable_with_weight_decay(name, shape, stddev, w_decay):
    ...
    var = tf.get_variable(name, shape, tf.truncated_normal_initializer(stddev=stddev, dtype=dtype))
...
def residual_RNNCell(input, hidden_state, hidden_conv_k, input_conv_k,
    ...
    #input  conv
    input_conv_k_shape = input_conv_k + [input_dim] + [out_dim*3]
    ...
    #hidden conv
    hidden_conv_k_shape = hidden_conv_k + [out_dim] + [out_dim*3]
    ...
    hidden_reset_conv_shape = [1, 1] + [out_dim, out_dim]
    ...
    #binarizer is stateless
        ...
        with tf.variable_scope('decoder_conv1', reuse=reuse) as scope:
            ...
            kernel = _variable_with_weight_decay('weights_decode_conv1',
                                                 ...
                                                 stddev=5e-2,
                                                 w_decay=0.0)
            decoder_conv1_out = tf.nn.conv2d(binarize_conv_sign, kernel,
                                             [1, 1, 1, 1], padding='SAME')
        #decode rnn1 8x8x256
        with tf.variable_scope('decoder_rnn1', reuse=reuse) as scope:
            ...
            decoder_rnn1_out, hidden_state_dec_rnn1 = residual_RNNCell(decoder_conv1_out,
                                                                       ...
                                                                       [1, 1, 1, 1])
        #depth to width
        tf.reshape(decoder_rnn1_out, [-1, 16, 16, 64])
        #decode rnn2
        with tf.variable_scope('decoder_rnn2', reuse=reuse) as scope:
            if i > 0: tf.get_variable_scope().reuse_variables()
            decoder_rnn2_out, hidden_state_dec_rnn2 = residual_RNNCell(decoder_conv1_out,
                                                                       ...
                                                                       [1, 1, 1, 1])
        #depth to width
        tf.reshape(decoder_rnn1_out, [-1, 32, 32, 64])
        #decode cnn2
        with tf.variable_scope('decoder_conv2', reuse=reuse) as scope:
            ...
            kernel = _variable_with_weight_decay('weights_decode_conv2',
                                                 ...
                                                 stddev=5e-2,
                                                 w_decay=0.0)
            decoder_conv2_out = tf.nn.conv2d(decoder_rnn2_out, kernel,
                                             [1, 1, 1, 1], padding='SAME')
        #calc residual images
        if i == 0:
            out_resi_images.append(copy.deepcopy(decoder_conv2_out))
        images = math_ops.sub(images, decoder_conv2_out)
        out_resi_images.append(copy.deepcopy(images))
...
#use a class
            ...
            """
            #input residual conv
            kernel_resi_conv = _variable_with_weight_decay('weights_residual_conv',
                                                           shape=[3, 3, 64, 768],
                                                           stddev=5e-2,
                                                           w_decay=0.0)
            resi_conv = tf.nn.conv2d(encode_conv1, kernel_resi_conv, [1, 2, 2, 1],
                                     padding='SAME')
            z_resi, r_resi, o_resi = array_ops.split(3, 3, resi_conv)
            #hidden conv
            kernel_hid_conv = _variable_with_weight_decay('weights_hidden_conv',
                                                          shape=[1, 1, 256, 768],
                                                          stddev=5e-2,
                                                          w_decay=0.0)
            hid_conv = tf.nn.conv2d(hidden_state_enc, kernel_hid_conv, [1, 1, 1, 1],
                                    padding='SAME')
            z_hid, r_hid, rc_hid = array_ops.split(3, 3, hid_conv)
            #calc z r
            z_gate = math_ops.add(z_resi, z_hid)
            ...
            r_gate = tf.nn.sigmoid(r_gate)
            #calc new_hidden
            zopp_mul_hidden = math_ops.sub(hidden_state_enc, math_ops.mul(z_gate, hidden_state_enc))
            #reset
            r_mul_hidden = math_ops.mul(r_gate, hidden_state_enc)
            kernel_reset_conv = _variable_with_weight_decay('weights_reset_conv',
                                                            shape=[1, 1, 256, 256],
                                                            stddev=5e-2,
                                                            w_decay=0.0)
            r_mul_hid_conv = tf.nn.conv2d(r_mul_hidden, kernel_reset_conv, [1, 1, 1, 1],
                                          padding='SAME')
            r_mul_hid_conv_tanh = tf.nn.tanh(math_ops.add(o_resi, r_mul_hid_conv))
            zr_mul_hid_conv_tanh = math_ops.mul(z_hid, r_mul_hid_conv_tanh)
            hid_rconnect_conv = math_ops.scalar_mul(FLAGS.hidden_rconnect_weight, rc_hid)
            #new hidden state
            hidden_state_enc = math_ops.add(math_ops.add(zopp_mul_hidden, zr_mul_hid_conv_tanh),
                                            hid_rconnect_conv)
            #input residual connection 1x1 kernel
            kernel_input_rc_conv = _variable_with_weight_decay('weights_input_rc_conv',
                                                               shape=[1, 1, 64, 256],
                                                               stddev=5e-2,
                                                               w_decay=0.0)
            rc_resi = tf.nn.conv2d(encode_conv1, kernel_input_rc_conv, [1, 1, 1, 1],
                                   padding='SAME')
            rc_resi = math_ops.scalar_mul(FLAGS.input_rconnect_weight, rc_resi)
            encode_rnn1_out = math_ops.add(hidden_state_enc, rc_resi)"""